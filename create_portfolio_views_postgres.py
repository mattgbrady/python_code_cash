...
def process_daily_data(conn):
    ...
    daily_transactions = created_df_from_postgres(conn,db_name='the_zoo.sti_daily_transactions')

    daily_transactions.drop(['id','created_at'], axis=1, inplace=True)

    min_date = min(daily_transactions['the_date'])

    current_date = date.today() + timedelta(days=1)

    num_days = current_date - min_date

    num_days = num_days.days

    
    date_array = np.arange(min_date,current_date)

    daily_transactions.set_index('the_date', inplace=True)

    daily_transactions.reset_index(date_array, inplace=True)
